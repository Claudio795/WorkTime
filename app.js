...
let adoProjects = [];  // [{id, name}]
let adoPbis     = [];  // [{id, title, projectName, state}]
let adoTasks    = {};  // {pbiId: [{id, title, pbiId, state}] | null (loading)}
let adoLastSync = null;
let adoProjectColors  = {}; // { projectName: '#hexcolor' }
let adoProjectAliases = {}; // { projectName: 'alias display' }
...
// ── ADO API
function adoHeaders() {
  const token = btoa(`:${adoConfig.pat}`);
...
async function adoFetchPbis() {
  const base = adoConfig.orgUrl.replace(/\/$/, '');
  const wiql = {
    query: `SELECT [System.Id],[System.Title],[System.TeamProject],[System.State]
            FROM WorkItems
  ...
  const result = await adoFetch(`${base}/_apis/wit/wiql?api-version=${ADO_API_VERSION}&$top=500`, {
  ...
  for (let i = 0; i < ids.length; i += 200) {
    ...
    const fields = 'System.Id,System.Title,System.TeamProject,System.State';
    const d = await adoFetch(`${base}/_apis/wit/workitems?ids=${batch.join(',')}&fields=${fields}&api-version=${ADO_API_VERSION}`);
    items.push(...(d.value || []));
  }
  return items.map(w => ({
    id: w.id,
    title: w.fields['System.Title'],
    projectName: w.fields['System.TeamProject'],
...
async function adoFetchTasksForPbi(pbiId) {
  ...
  const childIds = (item.relations || [])
    ...
    .map(r => { const m = r.url.match(/\/(\d+)$/); return m ? +m[1] : null; })
    .filter(Boolean);
  ...
  return (d.value || [])
    .filter(w => w.fields['System.WorkItemType'] === 'Task')
    .map(w => {
...
async function loadTasksForPbi(pbiId) {
  ...
  try {
    adoTasks[pbiId] = await adoFetchTasksForPbi(pbiId);
    saveAdoCache();
  } catch(e) {
    throw e;
  } finally {
    _fetchingPbis.delete(pbiId);
  }
}
  ...
  el.innerHTML = `<div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">Progetti — nome · colore</div>
    <div class="tag-list" style="max-height:220px;margin-bottom:0">
      ${adoProjects.map((p,i) => {
        ...
        const alias = adoProjectAliases[p.name] || '';
        return `<div class="tag-row" data-pi="${i}">
          ...
          <input class="tag-name-input-inline" type="text" maxlength="32" placeholder="${esc(p.name)}"
                 data-action="alias" data-pi="${i}" value="${esc(alias)}">
          <div class="tag-color-swatch" style="background:${color}">
            <input type="color" value="${color}" data-pi="${i}">
          </div>
...
async function saveAndSyncAdo() {
  ...
  status.textContent = '⟳ Recupero progetti…';

  try {
    adoProjects = await adoFetchProjects();
    status.textContent = `⟳ ${adoProjects.length} progetti — recupero PBI assegnati…`;
    adoPbis = await adoFetchPbis();
    adoTasks = {};
    adoLastSync = Date.now();
    saveAdoCache();
    status.style.color = 'var(--today-border)';
    status.textContent = `✓ ${adoProjects.length} progetti · ${adoPbis.length} PBI sincronizzati`;
    updateAdoCacheInfo();
    renderAdoProjectColors();
    render();
  } catch(e) {
    status.style.color = '#fa6c8e';
    status.textContent = `✗ ${e.message}`;
  } finally {
    btn.classList.remove('btn-spinning');
    btn.textContent = 'Salva & Sincronizza';
  }
}

// ── ACTIVITY TYPES (custom tags)
const DEFAULT_TYPES = [
...
function getTypeById(id) {
  return activityTypes.find(t => t.id === id) || { id, label: id, color: '#888' };
}
...
function normaliseSlot(s) {
  ...
  if (s.source === 'ado') {
    return { source:'ado', projectName:s.projectName||'', pbiId:s.pbiId||null,
             pbiTitle:s.pbiTitle||'', taskId:s.taskId||null, taskTitle:s.taskTitle||'',
             desc:s.desc||'', mins };
  }
  // custom or legacy (has .type)
  if (s.source === 'custom' || s.type) {
    return { source:'custom', typeId: s.typeId||s.type||'task', desc:s.desc||'', mins };
  }
  return null;
}
...
function makeMonthData(y, m) {
  return Array.from({length: daysInMonth(y,m)}, () => Array.from({length:9}, () => []));
}
...
function migrateData(entry) {
  ...
  const isOldFormat = Array.isArray(entry.data) && entry.data.length===2
    && Array.isArray(entry.data[0]) && Array.isArray(entry.data[0][0]);
  let flat;
  if (isOldFormat) {
    flat = Array.from({length:n}, (_,idx) => {
      const half=Math.floor(idx/14), d=idx%14, src=entry.data[half]?.[d];
      return Array.from({length:9}, (_,h) => normaliseCell(src?.[h]));
    });
  } else {
    flat = Array.from({length:n}, (_,idx) => {
      const src = entry.data[idx];
      return Array.from({length:9}, (_,h) => normaliseCell(src?.[h]));
    });
  }
  entry.data = flat;
...
function saveState() {
  try {
    ...
    if (ind) {
      ind.textContent = '✓ salvato'; ind.style.opacity='1';
      clearTimeout(ind._t);
...
function getItalianHolidays(year) {
  ...
  const fixed=[[0,1,"Capodanno"],[0,6,"Epifania"],[3,25,"Liberazione"],[4,1,"Festa del Lavoro"],
    [5,2,"Repubblica"],[7,15,"Ferragosto"],[10,1,"Ognissanti"],[11,8,"Immacolata"],[11,25,"Natale"],[11,26,"S. Stefano"]];
  const map={};
...
(function(){
  ...
  // Auto-sync ADO on every page load if credentials are already configured
  autoSyncAdo();
})();
...
async function autoSyncAdo() {
  ...
  try {
    ...
    adoProjects = await adoFetchProjects();
    adoPbis     = await adoFetchPbis();
    // Keep tasks cache but remove entries for PBIs no longer present
    ...
    console.warn('ADO auto-sync failed:', e.message);
...
function renderLegend() {
  const customHtml = activityTypes.map(t =>
    `<span class="legend-item"><span class="ldot" style="background:${t.color}"></span> ${esc(t.label)}</span>`
  ).join('');
  const adoHtml = adoProjects.slice(0, 6).map(p =>
    `<span class="legend-item"><span class="ldot" style="background:${projectColor(p.name)}"></span> ${esc(projectDisplayName(p.name))}</span>`
  ).join('');
...
function renderTable() {
  ...
  for (let d=0;d<nDays;d++) {
    ...
    html+=`<th class="${cls.trim()}" ${clickable?`onclick="selectDay(${d})"`:''}>
  ...
  for (let h=0;h<9;h++) {
    ...
    for (let d=0;d<nDays;d++) {
      ...
      if (holiday) {
        ...
        html+=`<td class="${tdCls}" data-d="${d}" data-h="${h}">${renderCellInner(slots,activeMonth,d,h)}</td>`;
      }
    }
    html+='</tr>';
  }
...
function slotBadgeText(s) {
  if (s.source==='ado') {
    const disp = s.projectName ? projectDisplayName(s.projectName) : 'ADO';
    const proj = disp.slice(0,6);
    const title = s.taskTitle || s.pbiTitle || '';
    return `${proj}${title?' · '+title:''}`;
  }
  return getTypeById(s.typeId).label;
}
...
function renderCellInner(slots, mi, d, h) {
  ...
  const bars = slots.map(s => {
    ...
    const badge=slotBadgeText(s);
    const hasTask = s.source==='ado' && s.taskId;
    const extraStyle = hasTask ? `;border-right:2px solid ${color}` : '';
    const taskDot = hasTask ? '◆ ' : '';
    return `<div class="slot-bar" style="height:${pct}%;background:${tagBg(color)};border-left:3px solid ${color}${extraStyle}">
      <span class="slot-badge" style="color:${color}">${taskDot}${s.mins<60?s.mins+'m ':''} ${esc(badge)}</span>
...
function updateStats() {
  ...
  m.data.forEach((dayHours,d) => {
    ...
    dayHours.forEach((slots,h) => {
      ...
      (slots||[]).forEach(s => {
        fattMins+=s.mins;
        if (s.source==='ado' && s.taskId) {
          scrumMins+=s.mins;
          if (s.pbiId && adoTasks[s.pbiId]===undefined) missingPbis.add(s.pbiId);
          if (!taskMap.has(s.taskId)) {
            const tasks=adoTasks[s.pbiId]||[];
            const t=tasks.find(t=>t.id===s.taskId);
            const exp=t?(t.estimate??null):null;
            taskMap.set(s.taskId,{logged:0,expected:exp!==null?Math.round(exp*60):null,title:s.taskTitle||String(s.taskId),project:projectDisplayName(s.projectName||'')});
          }
          taskMap.get(s.taskId).logged+=s.mins;
        }
  ...
  _taskCoverage=[...taskMap.values()];
  const withExp=_taskCoverage.filter(t=>t.expected!==null);
  const covered=withExp.filter(t=>t.logged>=t.expected).length;
  const total=_taskCoverage.length;
  const allOk=withExp.length>0&&covered===withExp.length;
  const coverageColor=total===0?'var(--muted)':allOk?'var(--today-border)':'#faa94c';
  const coverageLabel=withExp.length>0?`${covered}/${withExp.length}`:`${total}`;
  ...
  document.getElementById('statsBar').innerHTML=`
    ...
    ${total>0?`<div class="stat stat-clickable" onclick="toggleCoverageBar()"><div class="stat-value" style="color:${coverageColor}">${coverageLabel}</div><div class="stat-label">Task coperti</div></div>`:''}`;
  renderCoverageBar();
}
...
function renderCoverageBar() {
  ...
  if (!_taskCoverage.length) { bar.innerHTML='<div class="coverage-header"><span>Verifica copertura task</span><button class="btn btn-secondary btn-sm" onclick="toggleCoverageBar()">✕</button></div><span style="font-size:12px;color:var(--muted)">Nessun task ADO loggato</span>'; return; }
  const rows=_taskCoverage.map(t=>{
    const hasExp=t.expected!==null;
    const ok=hasExp&&t.logged>=t.expected;
    const diff=hasExp?t.expected-t.logged:null;
    const statusColor=!hasExp?'var(--muted)':ok?'var(--today-border)':'#faa94c';
    const statusIcon=!hasExp?'—':ok?'✓':`−${fmtH(diff)}`;
    return `<div class="coverage-row">
      <div class="coverage-task"><span class="coverage-project">${esc(t.project)}</span>${esc(t.title)}</div>
      <div class="coverage-nums">
        <span class="coverage-logged">${fmtH(t.logged)}</span>
        <span class="coverage-sep">/</span>
        <span class="coverage-expected">${t.expected!==null?fmtH(t.expected):'?'}</span>
        <span class="coverage-status" style="color:${statusColor}">${statusIcon}</span>
      </div>
    </div>`;
  }).join('');
  bar.innerHTML=`<div class="coverage-header"><span>Verifica copertura task</span><button class="btn btn-secondary btn-sm" onclick="toggleCoverageBar()">✕</button></div>${rows}`;
}
...
function renderDayStats() {
  ...
  // Collect per-project and per-type data
  ...
  const statBlock=(color,value,label)=>
    `<div class="stat"><div class="stat-value" style="color:${color}">${value}</div><div class="stat-label">${label}</div></div>`;
  ...
  const scrumBlocks=projEntries.filter(([,v])=>v.scrum>0)
    .map(([name,v])=>statBlock('var(--accent)',fmtH(v.scrum),esc(projectDisplayName(name).slice(0,14)))).join('');
  ...
  bar.innerHTML=`
    <div class="day-stats-header">
      <div class="day-stats-label">${dayStr}</div>
...
function defaultSlot() {
  if (adoProjects.length && adoPbis.length) {
    return {source:'ado',projectName:'',pbiId:null,pbiTitle:'',taskId:null,taskTitle:'',desc:'',mins:60};
  }
...
function renderPopoverSlots() {
  ...
  // restore desc values (avoid HTML escaping)
  popSlots.forEach((s,i)=>{
    const inp=document.querySelector(`#slotEditor .slot-desc-input[data-si="${i}"]`);
    if (inp) inp.value=s.desc||'';
  });
}
...
function renderSlotRow(s, i) {
  ...
  const adoHtml=`<div class="slot-ado-fields"${isAdo?'':' style="display:none"'}>
    <select class="slot-ado-project ado-select" data-si="${i}">
  ...
  const customHtml=`<div class="slot-custom-fields"${!isAdo?'':' style="display:none"'}>
    <div class="slot-type-row">
      ${activityTypes.map(t=>{
        ...
        return `<button class="ep-type-btn${sel?' sel':''}"
          style="background:${sel?tagBg(t.color):'var(--surface)'};border-color:${sel?t.color:'var(--border)'};color:${sel?t.color:'var(--muted)'}"
          data-si="${i}" data-typeid="${t.id}">${esc(t.label)}</button>`;
      }).join('')}
    </div>
  </div>`;

  return `<div class="slot-edit-row" data-si="${i}">
    ...
    <div class="slot-source-row">
      <button class="slot-src-btn${isAdo?' active':''}" data-si="${i}" data-src="ado">☁ ADO</button>
...
document.getElementById('editPopover').addEventListener('click', e => {
  ...
  if (el.dataset.typeid!==undefined && el.dataset.mins===undefined) {
    popSlots[si].typeId=el.dataset.typeid; renderPopoverSlots(); return;
  }
  ...
  if (el.classList.contains('slot-del-btn')) {
    popSlots.splice(si,1); renderPopoverSlots();
  }
});
...
document.getElementById('editPopover').addEventListener('change', async e => {
  ...
  if (el.classList.contains('slot-ado-project')) {
    ...
    if (pbi) {
      // Invalidate stale cache missing estimate/remaining fields
      const cached=adoTasks[pbi];
      if (Array.isArray(cached) && cached.length>0 && cached[0].completed===undefined) delete adoTasks[pbi];
      if (adoTasks[pbi]===undefined) {
        renderPopoverSlots();
        try { await loadTasksForPbi(pbi); }
        catch(err) { showToast('Errore task: '+err.message); }
      }
    }
    renderPopoverSlots();
...
function openDayPop(e, mi, d) {
  ...
  document.getElementById('dpTitle').innerHTML=
    `${WEEKDAYS[dt.getDay()]} ${dt.getDate()} ${MONTH_SHORT[dt.getMonth()]} <span>${dt.getFullYear()}</span>`;
...
function renderDpContent() {
  ...
  if (dpSource==='ado') {
    ...
    inner=activityTypes.map(t=>{
      ...
      return `<button class="dp-type-btn${sel?' sel':''}"
        style="background:${sel?tagBg(t.color):'var(--surface)'};border-color:${sel?t.color:'var(--border)'};color:${sel?t.color:'var(--muted)'}"
        data-dptype="${t.id}">${esc(t.label)}</button>`;
    }).join('');
  }
...
async function dpSetAdoPbi(v, title) {
  ...
  if (dpAdoPbiId!==null && adoTasks[dpAdoPbiId]===undefined) {
    renderDpContent();
    try { await loadTasksForPbi(dpAdoPbiId); } catch(e) {}
  }
  renderDpContent();
}
...
function dpAutoTo() {
  ...
  const slots=Math.ceil(hours);
  let count=0, h=from;
  while (h<HOURS.length) {
    if (h!==LUNCH_IDX) { count++; if (count>=slots) break; }
    h++;
  }
...
function clearDay() {
  ...
  for (let h=0;h<HOURS.length;h++) {
    if (h===LUNCH_IDX) continue;
    months[mi].data[d][h]=[]; updateCellDOM(mi,d,h);
  }
  closeDayPop(); updateStats(); renderDayStats(); saveState();
}
...
function renderTagList() {
  ...
  list.innerHTML=activityTypes.map((t,i)=>`
    <div class="tag-row" data-idx="${i}">
      ...
      <div class="tag-color-swatch" data-idx="${i}" style="background:${t.color}">
        <input type="color" data-action="color" data-idx="${i}">
      </div>
...
document.getElementById('tagList').addEventListener('focusout', e=>{
  ...
  setTimeout(()=>{
    if (_tagDeletePending||row.contains(document.activeElement)) return;
    const val=row.querySelector('.tag-name-input-inline').value.trim();
    if (val) activityTypes[+row.dataset.idx].label=val;
    row.classList.remove('editing');
  },150);
});
...
function slotText(s) {
  if (s.source==='ado') {
    ...
    const minsStr=s.mins<60?` (${s.mins}min)`:'';
    ...
    for (let h=0;h<9;h++) {
      ...
      for (let d=0;d<nDays;d++) {
        ...
        if (holiday) row.push(`🎉 ${holiday}`);
          ...
          const slots=m.data[d][h]||[];
          row.push(slots.length?slots.map(slotText).join(' | '):null);
        }
      }
      rows.push(row);
    }
...
function openMigraModal() {
  ...
  projSel.innerHTML = `<option value="">— Progetto —</option>` +
    adoProjects.map(p => `<option value="${esc(p.name)}">${esc(projectDisplayName(p.name))}</option>`).join('');
...
function migraProjectChanged() {
  ...
  pbiSel.innerHTML = `<option value="">— PBI —</option>` +
    pbis.map(p => `<option value="${p.id}" data-title="${esc(p.title)}">${esc(p.title)}</option>`).join('');
...
async function migraPbiChanged() {
  const pbiSel = document.getElementById('migraToPbi');
  const taskSel = document.getElementById('migraToTask');
  const loadingEl = document.getElementById('migraTaskLoading');
  const pbiId = pbiSel.value ? +pbiSel.value : null;
  taskSel.innerHTML = '<option value="">— Task —</option>'; taskSel.disabled = true;
  if (pbiId) {
    if (adoTasks[pbiId] === undefined) {
      loadingEl.style.display = 'block';
      try { await loadTasksForPbi(pbiId); }
      catch(e) { showToast('Errore task: ' + e.message); }
      finally { loadingEl.style.display = 'none'; }
    }
    const tasks = adoTasks[pbiId] || [];
    taskSel.innerHTML = `<option value="">— Task —</option>` +
      tasks.map(t => `<option value="${t.id}" data-title="${esc(t.title)}">${esc(t.title)}</option>`).join('');
...
function countMigraMatches() {
  ...
  months.forEach(m => m.data.forEach(day => day.forEach(slots =>
    (slots||[]).forEach(s => {
      if (s.source === 'custom' && s.typeId === typeId)
        if (!filter || (s.desc||'').toLowerCase().includes(filter)) n++;
    })
  )));
  return n;
}
...
function renderMigraPreview() {
  const count = countMigraMatches();
  const projectName = document.getElementById('migraToProject').value;
  const preview = document.getElementById('migraPreview');
  const applyBtn = document.getElementById('migraApplyBtn');
  if (count === 0) {
    preview.textContent = 'Nessun slot corrispondente'; preview.style.color = 'var(--muted)';
    applyBtn.disabled = true; applyBtn.textContent = 'Sostituisci';
  } else {
    preview.textContent = `${count} slot ${projectName ? 'pronti per la migrazione' : '— seleziona progetto destinazione'}`;
    preview.style.color = projectName ? 'var(--today-border)' : 'var(--accent)';
    applyBtn.disabled = !projectName;
    applyBtn.textContent = projectName ? `Sostituisci ${count} slot` : 'Sostituisci';
  }
}
...
function applyMigration() {
  ...
  const taskSel = document.getElementById('migraToTask');
  const pbiId = pbiSel.value ? +pbiSel.value : null;
  const pbiTitle = pbiSel.selectedOptions[0]?.dataset?.title || '';
  const taskId = taskSel.value ? +taskSel.value : null;
  const taskTitle = taskSel.selectedOptions[0]?.dataset?.title || '';

  let count = 0;
  months.forEach(m => m.data.forEach(day => day.forEach(slots => {
    (slots||[]).forEach((s, i) => {
      if (s.source === 'custom' && s.typeId === typeId)
        if (!filter || (s.desc||'').toLowerCase().includes(filter)) {
          slots[i] = { source:'ado', projectName, pbiId, pbiTitle, taskId, taskTitle, desc:s.desc, mins:s.mins };
          count++;
        }
    });
  })));
  ...
  const dest = [projectName, pbiTitle, taskTitle].filter(Boolean).join(' / ');
  showToast(`✓ ${count} slot migrati → ${dest}`, 4000);
}
...
(function() {
  ...
  container.addEventListener('input', e => {
    const el = e.target;
    if (el.type === 'color' && el.dataset.pi !== undefined) {
      const name = adoProjects[+el.dataset.pi]?.name; if (!name) return;
      adoProjectColors[name] = el.value;
      el.closest('.tag-color-swatch').style.background = el.value;
    }
    if (el.dataset.action === 'alias') {
      const row = el.closest('.tag-row');
      row.querySelector('.tag-name-label').textContent = el.value.trim() || (adoProjects[+el.dataset.pi]?.name || '');
    }
  });
  ...
  container.addEventListener('click', e => {
    ...
    if (reset) {
      const name = adoProjects[+reset.dataset.reset]?.name; if (!name) return;
      delete adoProjectColors[name]; delete adoProjectAliases[name];
      persistAdoConfig(); renderAdoProjectColors(); renderLegend(); updateStats(); renderTable();
      return;
    }
    const editBtn = e.target.closest('[data-action="edit-alias"]');
    if (editBtn) {
      const row = editBtn.closest('.tag-row');
      row.classList.add('editing');
      const inp = row.querySelector('.tag-name-input-inline');
  ...
  container.addEventListener('keydown', e => {
    const row = e.target.closest('.tag-row.editing'); if (!row) return;
    if (e.key === 'Enter' || e.key === 'Escape') {
      saveAlias(row, e.key === 'Escape' ? (adoProjectAliases[adoProjects[+row.dataset.pi]?.name] || '') : e.target.value);
    }
  });
  ...
  container.addEventListener('focusout', e => {
    ...
    setTimeout(() => {
      ...
      saveAlias(row, row.querySelector('.tag-name-input-inline').value);
    }, 120);
  });
})();
